...
# 0910 (G_UserRoleId)
-- 사용자-권한 매핑 (조인 엔티티)
DROP TABLE `user_roles`; -- 기존의 user_roles 테이블 제거
...
# 0901 (주문 관리 시스템)
-- 트랜잭션, 트리거, 인덱스, 뷰 학습
# products(상품), stocks(재고), orders(주문 정보), order_items(주문 상세 정보), order_logs(주문 기록 정보)
...
CREATE TRIGGER trg_after_ofder_insert
